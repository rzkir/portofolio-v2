...
const CHAT_STORAGE_KEY = "science";
...
export function createScienceAgentController(root: ParentNode): () => void {
  ...
  let sessionCategory: AgentPromptCategory | null = SCIENCE_AGENT_CATEGORY;
  ...
  function applyCategoryPreset(prompt: string) {
    sessionCategory = SCIENCE_AGENT_CATEGORY;
  ...
  const chatSession = setupAgentChatSession({
    ...
    defaultCategory: SCIENCE_AGENT_CATEGORY,
    chatMessages,
